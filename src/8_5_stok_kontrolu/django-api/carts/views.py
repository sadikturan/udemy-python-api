...
class AddToCartView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # var ise miktarı arttır, kontrol et.
        if not created:
            new_quantity = cart_item.quantity + quantity
            if new_quantity > product.stock:
                return Response({'error': f"Cannot add {quantity} items. Only {product.stock - cart_item.quantity} more items available."}, status=status.HTTP_400_BAD_REQUEST)
